...
/*  General Settings  */
	...
	add_option("tmpmela_logo_image_alt",'megashop-multistore'); // set logo image alt
    add_option("tmpmela_showsite_description", 'no'); // yes/no, control panel
	add_option("tmpmela_mob_logo_image", get_stylesheet_directory_uri() . "/images/megnor/mob-logo.png"); // set mob-logo image
	add_option("tmpmela_mob_logo_image_alt",'megashop-multistore'); // set logo image alt		
	...
	add_option("tmpmela_bodyfont_color","707070"); // body font color
	...
	add_option("tmpmela_button_color","232F3E"); // button color
	...
	add_option("tmpmela_header_top_bkg_color","232F3E"); // Header Top Background color		
	
	add_option("tmpmela_header_bottom_bkg_color","1B2530"); // header bottom background color		
	
	add_option("tmpmela_show_header_cms","yes"); // Show Header Services & contact
	add_option("tmpmela_header_cms_text","Free Shipping"); // Header CMS Text
	add_option("tmpmela_header_cms_text_color","FFFFFF"); // Header CMS Text Color
	add_option("tmpmela_contact_text","Need Help?"); // Contact Text
	add_option("tmpmela_contact_text_color","FFFFFF"); // Contact Text Color
	
	
	
	
    /*  Navigation Menu Setting  */
	add_option("tmpmela_navbar_category_title","All Categories"); // Sidebar Category Title
	add_option("tmpmela_categoty_title_text_color","FFFFFF"); // Header Category Title text color
	
	add_option("tmpmela_sidebar_category_bg_color","232f3e"); // Sidebar Category Background Color
	...
	add_option("tmpmela_sub_menu_texthover_color","ffd200"); // Sub Menu Text hover color
	...
	add_option("tmpmela_hoverlink_color","ffd200"); // link hover color
	
	/*  Sidebar setting for other pages  */
	add_option("tmpmela_page_sidebar","yes");
	
	/*  Footer Settings  */	
	add_option("tmpmela_footer_bkg_color","232F3E"); // Footer Background color	
...
function tmpmela_service_cms() {
	...
	<div class="service-text text1">'.stripslashes(get_option('tmpmela_header_cms_text')).'</div>
	</div>';
...
function tmpmela_excerpt($limit) 
{
      ...
      if (count($excerpt)>=$limit) {
        array_pop($excerpt);
        $excerpt = implode(" ",$excerpt).'.<div class="read-more"><a class="read-more-link" href="'.esc_url(get_permalink()).'">'.esc_html__( 'Read More' , 'megashop-multistore' ) .'</a></div>';
      } else {
...
function tmpmela_load_fonts() {
    ...
    */
	...
	$Poppins = _x( 'on', 'Poppins font: on or off', 'megashop-multistore' );
...
function tmpmela_search_form( $form ) {
    ...
    <div><label class="screen-reader-text" for="s">' . esc_attr_x( 'Search for:','label','megashop-multistore' ) . '</label>
    <input class="search-field" type="text" placeholder="'. esc_attr__( 'Search', 'megashop-multistore' ) .'" value="' . get_search_query() . '" name="s" id="s" />
    <input class="search-submit" type="submit" id="searchsubmit" value="'. esc_attr_x( 'Go','label','megashop-multistore' ) .'" />
    </div>
    </form>';
    return $form;
}
...
<li <?php comment_class(); ?> id="comment-<?php comment_ID(); ?>">
  <p>
    <?php esc_html_e( 'Pingback:', 'megashop-multistore' ); ?>
    <?php comment_author_link(); ?>
    <?php edit_comment_link( esc_html__( '(Edit)', 'megashop-multistore' ), '<span class="edit-link">', '</span>' ); ?>
  </p>
</li>
<?php
			break;
...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID(); ?>">
  <div id="comment-<?php comment_ID(); ?>" class="comment-body">
    ...
    <div class="author-content">
      ...
      <?php edit_comment_link( esc_html__( 'Edit', 'megashop-multistore' ), ' ' ); ?>
      <div class="clearfix"></div>
      <abbr class="published" title="<?php echo get_the_title() ?>"><?php printf( esc_html__( '%1$s at %2$s', 'megashop-multistore' ), get_comment_date(),  get_comment_time() ); ?></abbr> </div>
    ...
    <?php esc_html_e( 'Your comment is awaiting moderation.', 'megashop-multistore' ); ?>
    </em> <br />
    <?php endif; ?>
  </div>
  <!-- #comment-##  -->
...
function tmpmela_sticky_post() {
	if ( is_sticky() && is_home() && ! is_paged() )
		echo '<div class="meta-inner"><span class="sticky-post">' . esc_html__( 'Sticky', 'megashop-multistore' ) . '</span></div>';
}
endif;
if ( ! function_exists( 'tmpmela_categories_links' ) ) :
function tmpmela_categories_links() {
	// Translators: used between list items, there is a space after the comma.
	$categories_list = get_the_category_list( esc_html__( ', ', 'megashop-multistore' ) );
	if ( $categories_list ) {
...
function tmpmela_tags_links() {
	// Translators: used between list items, there is a space after the comma.
	$tag_list = get_the_tag_list( '', esc_html__( ', ', 'megashop-multistore' ) );
	if ( $tag_list ) {
...
function tmpmela_author_link() {
	...
	if ( 'post' == get_post_type() ) {
		printf( '<div class="meta-inner"><span class="author vcard"><i class="fa fa-pencil-square-o"></i><a class="url fn n" href="%1$s" title="%2$s" rel="author">'.esc_html__("by ",'megashop-multistore').'%3$s</a></span></div>',
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( esc_html__( 'View all posts by %s', 'megashop-multistore' ), get_the_author() ) ),
			get_the_author()
		);
	}
}
...
function tmpmela_comments_link() {
	...
	comments_popup_link( esc_html__( 'Leave a Comment', 'megashop-multistore' ), esc_html__( '1 Comment', 'megashop-multistore' ), esc_html__( '% Comments', 'megashop-multistore' ) );
	echo '</span></div>';
	endif; 
}
endif;
...
function tmpmela_entry_date( $echo = true ) {
	if ( has_post_format( array( 'chat', 'status' ) ) )
		$format_prefix = _x( '%1$s on %2$s', '1: post format name. 2: date', 'megashop-multistore' );
	else
		$format_prefix = '%2$s';
...
<time class="entry-date" datetime="%3$s">%4$s</time></a></span></div>',
		...
		esc_attr( sprintf( esc_html__( 'Permalink to %s', 'megashop-multistore' ), the_title_attribute( 'echo=0' ) ) ),
...
function tmpmela_shortcode_paging_nav() {
	...
	$links = paginate_links( array(
		...
		'prev_text' =>  wp_kses( __( '<i class="fa fa-angle-left"></i>', 'megashop-multistore' ),tmpmela_allowed_html()),
		'next_text' =>  wp_kses( __( '<i class="fa fa-angle-right"></i>', 'megashop-multistore' ),tmpmela_allowed_html()),
	) );
	if ( $links ) :
...
function tmpmela_out_of_stock() {
 global $product;
 	if ( !$product->is_in_stock() ) {
        echo '<div class="soldout_wrapper"><span class="soldout">'.esc_html__('SOLD OUT','megashop-multistore').'</span></div>';
    }
}
endif;
...
{
  	if ( $rating > 0 ) {
    	$title = sprintf( __( 'Rated %s out of 5', 'megashop-multistore' ), $rating );
  	} else {
    	$title = 'Not yet rated';
    	$rating = 0;
  	}
  	$rating_html  = '<div class="star-rating" title="' . $title . '">';
    $rating_html .= '<span style="width:' . (($rating / 5) * 100) . '%"><strong class="rating">' . $rating . '</strong> ' . esc_attr__('out of 5', 'megashop-multistore') . '</span>';
  	$rating_html .= '</div>';
  	return $rating_html;
	}
...
$label     = $req ? '*' : ' ' . __( '(optional)', 'megashop-multistore' );
$aria_req  = $req ? "aria-required='true'" : '';
$fields['author'] =
'<p class="comment-form-author comment-block">
<input id="author" name="author" type="text" placeholder="' . esc_attr__( "Name*", "megashop-multistore" ) . '" value="' . esc_attr( $commenter['comment_author'] ) .
'" size="30" ' . $aria_req . ' />
</p>';
$fields['email'] =
'<p class="comment-form-email comment-block">
<input id="email" name="email" type="email" placeholder="' . esc_attr__( "E-mail*", "megashop-multistore" ) . '" value="' . esc_attr( $commenter['comment_author_email'] ) .
'" size="30" ' . $aria_req . ' />
</p>';
$fields['url'] =
'<p class="comment-form-url comment-block">
<input id="url" name="url" type="url"  placeholder="' . esc_attr__( "Website", "megashop-multistore" ) . '" value="' . esc_attr( $commenter['comment_author_url'] ) .
'" size="30" />
</p>';
return $fields;
}
