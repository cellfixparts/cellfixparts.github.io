...
function tmpmela_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		<h3 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'megashop-multistore' ); ?></h3>
...
function tmpmela_post_nav() {
	...
	<nav class="navigation post-navigation">
		<h3 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'megashop-multistore' ); ?></h3>
		<div class="nav-links">
			<?php
			if ( is_attachment() ) :
				previous_post_link( '%link',  wp_kses( __( '<span class="meta-nav">Published In</span>%title', 'megashop-multistore' ),tmpmela_allowed_html()) );
			else :
				previous_post_link( '%link', wp_kses( __( '<i class="fa fa-chevron-left"></i>%title', 'megashop-multistore' ),tmpmela_allowed_html()) );
				next_post_link( '%link', wp_kses( __( '%title<i class="fa fa-chevron-right"></i>', 'megashop-multistore' ),tmpmela_allowed_html()) );
			endif;
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
...
function tmpmela_posted_on() {
	if ( is_sticky() && is_home() && ! is_paged() ) {
		echo '<span class="featured-post">' . esc_html__( 'Sticky', 'megashop-multistore' ) . '</span>';
	}
