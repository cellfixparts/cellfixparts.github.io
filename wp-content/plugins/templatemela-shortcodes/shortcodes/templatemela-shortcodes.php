...
if ( $blog_array->have_posts() ):
	...
	if( empty ( $src ) || $src == 'image_not_specified' ):
		...
		$output .= '<div class="post-description">'.tmpmela_blog_post_excerpt(90).'</div>';
		$output .= '</div>';
...
$output .=	'</div></div>';
	...
	$output .= '<div class="no-result">'.esc_html__('No results found...', 'megashop-multistore').'</div>';
endif;
return $output;
}
...
function shortcode_woo_category_slider($atts, $content = null, $code) {
	...
	function shortcode_cms_block($atts, $content = null, $code) {
		...
		$output .='<div class="cms-banner-item '.$cms_text_align.' '.$classname.' '.$noimage.'"><div class="cms-banner-inner">'.$cmsimg.'<span class="static-wrapper">'.$cmstext1.''.$cmstext2.''.$link_text.'</span></div></div>';
...
// Display the submit button.
	...
	function shortcode_portfolio_slider_container($atts, $content = null, $code) {
		...
		if($count > 0):
			...
			while (have_posts()) : the_post();
				...
				$output .= '<a href="'.get_permalink().'" title="'.esc_html__('Click to view read more', 'megashop-multistore').'" class="icon"><i class="fa fa-link"></i></a>';							
				$output .= '</div>';
				$output .= '</div>';
				$output .= '</div>';
				...
				$output .= '<div class="read-more"><a href="'.get_permalink().'" title="'.get_the_title().'">'.esc_html__('Read more', 'megashop-multistore').'</a></div>';				
				$output .= '</div>';
				$output .= '</div>';
				$i++;
			endwhile;
			...
			$output .= '<div class="no-result">'.esc_html__(''.esc_html__('No results found...', 'megashop-multistore').'', 'megashop-multistore').'</div>';
		endif;
		return $output;
	}
	...
	function shortcode_woo_products_container($atts, $content = null, $code) {
		...
		if($type == "grid") {  
			...
			$output .=	'<div class="loadgridlist-wrapper"><button class="woocount loadgridlist">'.esc_html__('View More Products', 'megashop-multistore').'</button></div>';		
		}
	...
	function shortcode_custom_testimonials($atts, $content = null, $code) {
		...
		if($testimonial_count > 0 ):
			...
			foreach($testimonial_array as $post) : setup_postdata($post);	
				...
				$output .= '<div class="read-more"><a title="'.get_the_title().'" href="'.get_permalink().'" >'.esc_html__('read more', 'megashop-multistore').'</a></div>';
				$output .= '</div></div>';
...
function tmpmela_wp_get_attachment_link( $id = 0, $size = 'thumbnail', $permalink = true, $icon = false, $text = false ) {
	...
	if ( empty( $_post ) || ( 'attachment' != $_post->post_type ) || ! $url = wp_get_attachment_url( $_post->ID ) )
		return __( 'Missing Attachment', 'megashop-multistore' );
